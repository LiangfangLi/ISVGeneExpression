%% To do: validate specificity of HAR-BRAIN genes based on BRAIN gene.
...
% load BRAIN genes expression matrix
load('BRAIN_gene_expression.mat')
% 415 HAR-BRAIN gene symbol
load('HAR_BRAIN_symbol.mat')
% delte 415 HAR-BRAIN gene from 2979 BRAIN gene list
BRAIN_dlt_HARBRAIN_gene_expression= BRAIN_gene_expression(~ismember(BRAIN_gene_expression.Allen_Gene_Symbol,HAR_BRAIN_symbol),:);
save(fullfile(ReplicationFolder,'BRAIN_dlt_HARBRAIN_gene_expression.mat'),'BRAIN_dlt_HARBRAIN_gene_expression')
size(BRAIN_dlt_HARBRAIN_gene_expression)

%%  random select 415 gene from BRAIN gene(delete HAR-BRAIN) to repeate PLS regression
load('response_var_file.mat')
output_dir = fullfile(ReplicationFolder,'\Specificity\BRAIN')
[permutate_r1_PLS1,permutate_r2_PLS2] = PLS_random_select_gene(response_var_file,BRAIN_dlt_HARBRAIN_gene_expression,2,10000,output_dir);
